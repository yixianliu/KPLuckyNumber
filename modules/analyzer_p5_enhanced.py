...
class P5AnalyzerEnhanced:
    ...
    def __init__(self, history_data, trend_data=None):
        ...
        logger.info(f'分析器初始化完成，历史数据: {len(history_data)} 条，走势数据: {len(trend_data)} 条')
    ...
    def analyze_omission_trend(self, position=None, number=None, recent_periods=50):
        ...
        Args:
            position: 指定位置（0-4），None则分析所有位置
            number: 指定号码（0-9），None则分析所有号码
        ...
        positions = [position] if position is not None else range(5)
        numbers = [number] if number is not None else range(10)
        
        for pos in positions:
            pos_name = ['wan', 'qian', 'bai', 'shi', 'ge'][pos]
            results[pos] = {}
    ...
    def _classify_omission_level(self, current, average, maximum):
        ...
        avg_ratio = average / maximum if maximum > 0 else 0
    ...
    def calculate_enhanced_probability(self, position, number, 
        ...
        # 归一化到合理范围
        final_prob = min(max(combined_prob * 0.1, 0.02), 0.3)
    ...
    def _generate_combinations(self, optimal_data, max_combinations=10):
        ...
        top_numbers_per_pos = []
        for pos in range(5):
            top3 = [item['number'] for item in optimal_data[pos]['top_numbers'][:3]]
            top_numbers_per_pos.append(top3)
...
def test_analyzer():
    ...
    np.random.seed(42)
    for i in range(100):
        numbers = list(np.random.randint(0, 10, 5))
        history_data.append({
            'issue': f'2026{i:03d}',
            'numbers': numbers
        })
    ...
    for i in range(50):
        ...
        omissions = {}
        for pos_idx, pos_name in enumerate(['wan', 'qian', 'bai', 'shi', 'ge']):
            pos_omissions = {}
            for num in range(10):
    ...
    # 创建分析器
    analyzer = P5AnalyzerEnhanced(history_data, trend_data)
